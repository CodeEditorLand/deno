	...
	impl Build {
		pub fn setup() -> Build {
			let gn_mode = if cfg!(target_os = "windows") {
				// On Windows, we need to link with a release build of libdeno,
				// because rust always uses the release CRT.
				// TODO(piscisaureus): make linking with debug libdeno possible.
				String::from("release")
			} else {
				// Cargo sets PROFILE to either "debug" or "release", which
				// conveniently matches the build modes we support.
			...
			// Tell Cargo when to re-run this file. We do this first, so these
			// directives can take effect even if something goes wrong later
			...
			// Unfortunately we can't detect whether we're being run by `cargo
			// check`.
			...
			if check_only {
				// Enable the 'check_only' feature, which enables some
				// workarounds in the rust source code to compile
				// successfully without a bundle and snapshot
				println!("cargo:rustc-cfg=feature=\"check-only\"");
			}

			Build { gn_out_dir, gn_out_path, check_only, gn_mode, root }
		}
		...
		pub fn run(&self, gn_target:&str) {
			...
			let ninja = if !cfg!(target_os = "windows") {
				...
				// Windows needs special configuration. This is similar to the
				// function of python_env() in //tools/util.py.
