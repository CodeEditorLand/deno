use std::{env::current_dir, error::Error, fmt, path::PathBuf};
...
impl ModuleSpecifier {
	fn is_dummy_specifier(specifier:&str) -> bool { specifier == "<unknown>" }
	...
	pub fn resolve_import(
		...
		let url = match Url::parse(specifier) {
			// 1. Apply the URL parser to specifier. If the result is not
			//    failure, return he result.
			Ok(url) => url,

			// 2. If specifier does not start with the character U+002F SOLIDUS
			//    (/), the two-character sequence U+002E FULL STOP, U+002F
			//    SOLIDUS (./), or the three-character sequence U+002E FULL
			//    STOP, U+002E FULL STOP, U+002F SOLIDUS (../), return failure.
			...
			Err(ParseError::RelativeUrlWithoutBase) => {
				let base = if ModuleSpecifier::is_dummy_specifier(base) {
					...
					// the parent directory (appending trailing slash does not
					// work)
			...
			// If parsing the specifier as a URL failed for a different reason
			// than it being relative, always return the original error. We
			// don't want to return `ImportPrefixMissing` or `InvalidBaseUrl`
			// if the real problem lies somewhere else.
	...
	/// file path; if it is a relative path it's resolved relative to the
...
impl From<Url> for ModuleSpecifier {
	fn from(url:Url) -> Self { ModuleSpecifier(url) }
}
...
mod tests {
	...
	fn test_resolve_import() {
		let tests = vec![
			...
			// This test is disabled because the url crate does not follow the
			// spec, dropping the server part from the final result.
			// (
	...
	fn test_resolve_import_error() {
		...
		for (specifier, base, expected_err) in tests {
			let err =
				ModuleSpecifier::resolve_import(specifier, base).unwrap_err();
	...
	fn test_resolve_url_or_path() {
		...
		if cfg!(target_os = "windows") {
			...
			tests.extend(vec![
				...
				// (r"\\.\C:\deno\tests\006_url_imports.ts",
				// expected_url.to_string()), Not supported:
				// `Url::from_file_path()` performs the wrong conversion.
			...
			tests.extend(vec![
				...
				(r"\\server/share/deno/cool", expected_url.to_string()),
				// Not supported: `Url::from_file_path()` performs the wrong
				// conversion. (r"//server/share/deno/cool",
				// expected_url.to_string()),
			]);
		} else {
			// Absolute local path.
			...
			// Relative local path.
			let expected_url =
				format!("file://{}/tests/006_url_imports.ts", cwd_str);
			tests.extend(vec![
	...
	fn test_resolve_url_or_path_error() {
		...
		for (specifier, expected_err) in tests {
			let err =
				ModuleSpecifier::resolve_url_or_path(specifier).unwrap_err();
			assert_eq!(err, expected_err);
		}
	}
