...
/// by implementing dispatcher function that takes control buffer and optional
/// zero copy buffer as arguments. An async Op corresponds exactly to a Promise
/// in JavaScript.
pub struct Isolate {
...
impl Isolate {
	/// startup_data defines the snapshot or script used at startup to
	...
	/// Called whenever Deno.core.dispatch() is called in JavaScript.
	/// zero_copy_buf corresponds to the second argument of
	/// Deno.core.dispatch().
	///
	/// Requires runtime to explicitly ask for op ids before using any of the
	/// ops.
	...
	/// the caller to wrap the V8Exception into an error. By default this
	/// callback is set to CoreJSError::from_v8_exception.
	...
	extern fn pre_dispatch(
		...
		match op {
			Op::Sync(buf) => {
				// For sync messages, we always return the response via
				// Deno.core.send's return value. Sync messages ignore the
				// op_id.
				let op_id = 0;
				isolate
	...
	#[inline]
	fn as_raw_ptr(&self) -> *const c_void { self as *const _ as *const c_void }
	...
	/// Takes a snapshot. The isolate should have been created with
	/// will_snapshot set to true.
	///
	/// ErrBox can be downcast to a type that exposes additional information
	/// about the V8 exception. By default this type is CoreJSError, however it
	/// may be a different type if Isolate::set_js_error_create() has been
		...
		loop {
			match self.pending_dyn_imports.poll_next_unpin(cx) {
				Poll::Pending | Poll::Ready(None) => {
					// There are no active dynamic import loaders, or none are
					// ready.
				...
				Poll::Ready(Some((
					...
					// A module (not necessarily the one dynamically imported)
					// has been fetched. Create and register it, and if
				...
				Poll::Ready(Some((Some(Err((dyn_import_id, err))), _))) => {
					// A non-javascript error occurred; this could be due to a
...
impl Drop for LockerScope {
	fn drop(&mut self) { unsafe { libdeno::deno_unlock(self.libdeno_isolate) } }
}
		...
		loop {
			...
			match inner.pending_ops.poll_next_unpin(cx) {
				...
				Poll::Ready(Some(Ok((op_id, buf)))) => {
					...
					if !successful_push {
						// If we couldn't push the response to the shared queue,
						// because there wasn't enough size, we will
						// return the buffer via the legacy route, using
						// the argument of deno_respond.
...
/// IsolateHandle is a thread safe handle on an Isolate. It exposed thread safe
/// V8 functions.
#[derive(Clone)]
...
pub mod tests {
	...
	#[test]
	fn dyn_import_err() {
		// Test an erroneous dynamic import where the specified module isn't
		// found.
		run_in_task(|cx| {
	...
	fn dyn_import_err2() {
		...
		// Import multiple modules to demonstrate that after failed dynamic
		// import another dynamic import can still be run
		run_in_task(|cx| {
			let count = Arc::new(AtomicUsize::new(0));
			let count_ = count.clone();
			let mut isolate = Isolate::new(StartupData::None, false);
	...
	fn terminate_execution() {
        ...
        "#,
			...
			// execute() terminated, which means terminate_execution() was
			// successful.
			tx.send(true).ok();
	...
	fn overflow_res_async() {
		run_in_task(|_cx| {
			// TODO(ry) This test is quite slow due to memcpy-ing 100MB into JS.
