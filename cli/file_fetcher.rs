...
impl SourceFileFetcher {
	...
	pub fn fetch_source_file_async(
		...
		// Check if this file was already fetched and can be retrieved from in-process
		// cache.
	...
	fn fetch_cached_remote_source(
		...
		if let Some(redirect_to) = source_code_headers.redirect_to {
			...
			// filename /Users/kun/Library/Caches/deno/deps/https/import-meta.now.sh/
			// redirect.js redirect_to https://import-meta.now.sh/sub/final1.js
			// real_filename
			// /Users/kun/Library/Caches/deno/deps/https/import-meta.now.sh/sub/final1.js real_module_name = https://import-meta.now.sh/sub/final1.js
			let redirect_url = Url::parse(&redirect_to).expect("Should be valid URL");

			// Recurse.
			// TODO(bartlomieju): I'm pretty sure we should call `fetch_remote_source_async`
			// here. Should we expect that all redirects are cached?
...
impl SourceCodeHeaders {
	...
	pub fn to_json_string(
		...
		// TODO(kevinkassimo): consider introduce serde::Deserialize to make things
		// simpler. This is super ugly at this moment...
...
mod tests {
	...
	fn test_get_source_code_multiple_downloads_of_same_file() {
		...
		// `use_disk_cache` is set to false, this can be verified using source header
		// file creation timestamp (should be the same as after first download)
		tokio_util::run(fetcher.fetch_source_file_async(&specifier, None).then(|r| {
			assert!(r.is_ok());
	...
	fn test_get_source_code_5() {
		...
		let fut = fetcher
			...
			.then(move |result| {
				...
				// redirect target shouldn't be downloaded again. It can be verified using
				// source header file creation timestamp (should be the same as after first
				// `get_source_file`)
				fetcher
