...
use crate::{deno_error, deno_error::DenoError, version};
		...
		.and_then(
			...
			> {
				if response.status().is_redirection() {
					...
					let new_url =
						resolve_url_from_location(&url, location_string);
					// Boxed trait object turns out to be the savior for 2+
					// types yielding same results.
				...
				let content_type =
					response.headers().get(CONTENT_TYPE).map(|content_type| {
