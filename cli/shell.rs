...
/// An abstraction around a `Write`able object that remembers preferences for
/// output verbosity and color.
pub struct Shell {
	/// the `Write`able object, either with or without color support
	/// (represented by different enum variants)
	err:ShellOut,
...
enum ShellOut {
	...
	Stream { stream:StandardStream, tty:bool, color_choice:ColorChoice },
}
...
impl Shell {
	/// Creates a new shell (color choice and verbosity), defaulting to 'auto'
	/// color and verbose output.
	pub fn new() -> Shell {
		Shell {
			err:ShellOut::Stream {
	...
	// Creates a shell from a plain writable object, with no color, and max
	// verbosity. pub fn from_write(out: Box<dyn Write>) -> Shell {
	// Shell {
	...
	/// even if the color choice has been set to something else.
...
impl ShellOut {
	...
	/// max width that will right align is 12 chars.
	fn print(
		&mut self,
		status:&dyn fmt::Display,
		message:Option<&dyn fmt::Display>,
...
mod imp {
	...
	pub fn stderr_width() -> Option<usize> { None }
}
