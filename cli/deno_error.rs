...
use std::{self, env::VarError, error::Error, fmt, io};

use deno::{AnyError, ErrBox, ModuleResolutionError};
...
impl DenoError {
	pub fn new(kind:ErrorKind, msg:String) -> Self { Self { kind, msg } }
}
...
impl GetErrorKind for JSError {
	fn kind(&self) -> ErrorKind { ErrorKind::JSError }
}

impl GetErrorKind for Diagnostic {
	fn kind(&self) -> ErrorKind { ErrorKind::Diagnostic }
}

impl GetErrorKind for ImportMapError {
	fn kind(&self) -> ErrorKind { ErrorKind::ImportMapError }
}
...
impl GetErrorKind for dyn AnyError {
	fn kind(&self) -> ErrorKind {
		...
		#[cfg(not(unix))]
		fn unix_error_kind(_:&dyn AnyError) -> Option<ErrorKind> { None }

		None.or_else(|| self.downcast_ref::<DenoError>().map(Get::kind))
...
mod tests {
	...
	fn io_error() -> io::Error { io::Error::from(io::ErrorKind::NotFound) }

	fn url_error() -> url::ParseError { url::ParseError::EmptyHost }

	fn import_map_error() -> ImportMapError {
		ImportMapError { msg:"an import map error".to_string() }
	}
