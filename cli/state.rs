...
impl Clone for ThreadSafeState {
	fn clone(&self) -> Self { ThreadSafeState(self.0.clone()) }
}
...
impl ThreadSafeState {
	...
	/// This is a band-aid for transition to `Isolate.register_op` API as most
	/// of our ops require `state` argument.
...
impl Loader for ThreadSafeState {
	...
	fn load(
		...
		let fut = self
			...
			.map_ok(|compiled_module| {
				deno::SourceCodeInfo {
					// Real module name, might be different from initial
					// specifier due to redirections.
