...
pub fn resolve_from_cwd(path:&str) -> Result<(PathBuf, String), ErrBox> {
	...
	// `/deno/dev/./tests/`. On the other hand joining `/dev/deno/" with "./tests"
	// using `Url` yields "/dev/deno/tests"
	// - and that's what we want.
	// There exists similar method on `PathBuf` - `PathBuf.canonicalize`, but the
