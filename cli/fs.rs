...
pub fn resolve_from_cwd(path:&str) -> Result<(PathBuf, String), ErrBox> {
	...
	// Joining `/dev/deno/" with "./tests" using `PathBuf` yields
	// `/deno/dev/./tests/`. On the other hand joining `/dev/deno/" with
	// "./tests" using `Url` yields "/dev/deno/tests"
	// - and that's what we want.
	// There exists similar method on `PathBuf` - `PathBuf.canonicalize`, but
	// the problem is `canonicalize` resolves symlinks and we don't want that.
