...
//! This mod provides functions to remap a deno::V8Exception based on a source
//! map
...
fn builtin_source_map(_:&str) -> Option<Vec<u8>> { None }
...
pub fn apply_source_map<G:SourceMapGetter>(
	...
	V8Exception {
		...
		start_column,
		end_column,
		// These are difficult to map to their original position and they are
		// not currently used in any output, so we don't remap them.
...
mod tests {
	...
	fn v8_exception_apply_source_map_2() {
		...
		// Because this is accessing the live bundle, this test might be more
		// fragile
