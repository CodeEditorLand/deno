...
// Some deserializer fields are only used on Unix and Windows build fails
// without it
#[cfg(unix)]
use std::os::unix::fs::PermissionsExt;
use std::{convert::From, fs, path::PathBuf, time::UNIX_EPOCH};
...
#[cfg(any(unix))]
fn get_mode(perm:&fs::Permissions) -> u32 { perm.mode() }

#[cfg(not(any(unix)))]
fn get_mode(_perm:&fs::Permissions) -> u32 { 0 }
