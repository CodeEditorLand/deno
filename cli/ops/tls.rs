...
impl Drop for TlsListenerResource {
	fn drop(&mut self) { self.wake_task(); }
}
...
impl TlsListenerResource {
	...
	pub fn track_task(&mut self, cx:&Context) -> Result<(), ErrBox> {
		// Currently, we only allow tracking a single accept task for a
		// listener. This might be changed in the future with multiple
...
pub fn accept_tls(state:&ThreadSafeState, rid:ResourceId) -> AcceptTls {
	AcceptTls { accept_state:AcceptTlsState::Pending, rid, state:state.clone() }
}
