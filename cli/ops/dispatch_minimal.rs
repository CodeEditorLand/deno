...
use deno::{Buf, CoreOp, ErrBox, Op, PinnedBuf};
use futures::future::FutureExt;

use crate::{deno_error::GetErrorKind, msg::ErrorKind};
...
where
	...
	move |control:&[u8], zero_copy:Option<PinnedBuf>| {
		...
		if is_sync {
			// Warning! Possible deadlocks can occur if we try to wait for a
			// future while in a future. The safe but expensive alternative
			// is to use tokio_util::block_on.
			// This block is only exercised for readSync and writeSync, which I
			// think works since they're simple polling futures.
