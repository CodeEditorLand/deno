...
import { assert, assertEquals, assertThrows, test } from "./test_util.ts";
...
test(function urlModifications(): void {
	...
	assertEquals(
		url.href,
		"https://foo:qux@foo.bar/foo/bar%qat?foo=bar&baz=12",
	);
	...
	assertEquals(
		url.href,
		"http://foo:qux@foo.bar/foo/bar%qat?foo=bar&foo=baz",
	);
