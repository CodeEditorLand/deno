...
import { type Diagnostic, fromTypeScriptDiagnostic } from "./diagnostics.ts";
...
class SourceFile {
	...
	private static _specifierCache: Map<string, Map<string, SourceFile>> =
		new Map();
		...
		try {
			...
			if (this._requestType === CompilerRequestType.Bundle) {
				...
				if (fileName.endsWith(".map")) {
					...
					assert(
						false,
						"Trying to cache unhandled file type " + fileName,
					);
				}
			}
		} catch (e) {
...
window.compilerMain = function compilerMain(): void {
	...
	window.onmessage = async ({
		...
		const host = new Host(
			request.type,
			resolvedRootModules,
			request.type === CompilerRequestType.Bundle
				? request.outFile
				: undefined,
		);
