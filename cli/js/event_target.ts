...
export class EventTarget implements domTypes.EventTarget {
	...
	public removeEventListener(
		...
		const normalizedOptions: domTypes.EventListenerOptions =
			eventTargetHelpers.normalizeEventHandlerOptions(options);
		...
		if (
			...
			for (let i = eventImpl.path.length - 1; i >= 0; --i) {
				...
				if (tuple.target === null) {
					eventTargetHelpers.invokeEventListeners(
						targetImpl,
						tuple,
						eventImpl,
					);
			...
			for (let i = 0; i < eventImpl.path.length; i++) {
				...
				if (
					(eventImpl.eventPhase ===
						domTypes.EventPhase.BUBBLING_PHASE &&
						eventImpl.bubbles) ||
					eventImpl.eventPhase === domTypes.EventPhase.AT_TARGET
				) {
					eventTargetHelpers.invokeEventListeners(
