...
import type { AbortSignal, QueuingStrategySizeCallback } from "../dom_types.ts";
...
>(obj: O, methodName: K, extraArgs: any[]): any {
	...
	return (...fnArgs: any[]): any =>
		promiseCall(method, obj, fnArgs.concat(extraArgs));
}
...
export enum ControlledPromiseState {
	Pending = 0,
	Resolved = 1,
	Rejected = 2,
}
...
export function createControlledPromise<V>(): ControlledPromise<V> {
	...
	conProm.promise = new Promise<V>((resolve, reject) => {
		conProm.resolve = (v?: V): void => {
			conProm.state = ControlledPromiseState.Resolved;
			resolve(v);
		};
		conProm.reject = (e?: ErrorResult): void => {
