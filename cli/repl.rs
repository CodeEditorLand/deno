...
// Work around the issue that on Windows, `struct Editor` does not
// implement the `Send` trait, because it embeds a windows HANDLE which is
// a type alias for *mut c_void. This value isn't actually a pointer and
// there's nothing that can be mutated through it, so hack around it. TODO:
// a prettier solution.
#[cfg(windows)]
...
impl<T:rustyline::Helper> Editor<T> {
	pub fn new() -> Editor<T> { Editor { inner:rustyline::Editor::<T>::new() } }
}
...
impl<T:rustyline::Helper> Deref for Editor<T> {
	...
	fn deref(&self) -> &rustyline::Editor<T> { &self.inner }
}

#[cfg(windows)]
impl<T:rustyline::Helper> DerefMut for Editor<T> {
	fn deref_mut(&mut self) -> &mut rustyline::Editor<T> { &mut self.inner }
}
...
impl Drop for Repl {
	fn drop(&mut self) { self.save_history().unwrap(); }
}
