...
mod tests {
	...
	fn parse_specifier_keys_relative() {
		// Should absolutize strings prefixed with ./, ../, or / into the corresponding
		// URLs..
		let json_map = r#"{
      "imports": {
        "./foo": "/dotslash",
	...
	fn resolve_unmapped_bad_specifiers() {
		...
		// Should fail for string not parseable as absolute URLs and not starting with
		// ./, ../ or /.
    ...
    "imports": {
      ...
      "%2F": "/lib/percent2f.mjs"
		...
		// Should fail for attempting to get a submodule of something not declared with
		// a trailing slash.
    ...
    "imports": {
      ...
      "../test": "/lib/test2.mjs"
		...
		// Should use the last entry's address when URL-like specifiers parse to the
		// same absolute URL.
		//
