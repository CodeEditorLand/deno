use std::{cmp::Ordering, error::Error, fmt, fs, io};
...
impl ImportMapError {
	pub fn new(msg:&str) -> Self { ImportMapError { msg:msg.to_string() } }
}

impl fmt::Display for ImportMapError {
	fn fmt(&self, f:&mut fmt::Formatter) -> fmt::Result { f.pad(&self.msg) }
}

impl Error for ImportMapError {}

// NOTE: here is difference between deno and reference implementation - deno
// currently  can't resolve URL with other schemes (eg. data:, about:, blob:)
...
mod tests {
	...
	fn parse_specifier_keys_relative() {
		// Should absolutize strings prefixed with ./, ../, or / into the
		// corresponding URLs..
		let json_map = r#"{
      "imports": {
        "./foo": "/dotslash",
      ...
      "imports": {
		...
		assert_eq!(
			import_map.imports.get("https://base.example/path1/").unwrap()[0],
			"https://base.example/dotdotslash/".to_string()
		);
		assert_eq!(
		...
		// Should treat percent-encoded variants of ./, ../, or / as bare
		// specifiers..
		let json_map = r#"{
      "imports": {
        "%2E/": "/dotSlash1/",
	...
	fn parse_specifier_keys_absolute() {
      ...
      "imports": {
		...
		// Should parse absolute URLs, treating unparseable ones as bare
		// specifiers..
		let json_map = r#"{
      "imports": {
	...
	fn parse_addresses_absolute_with_fetch_schemes_arrays() {
		// Should only accept absolute URL addresses with fetch schemes inside
		// arrays..
		let json_map = r#"{
      "imports": {
        "http": ["http://good/"],
	...
	fn resolve_unmapped_bad_specifiers() {
		...
		// Should fail for string not parseable as absolute URLs and not
		// starting with ./, ../ or /.
    ...
    "imports": {
      ...
      "%2F": "/lib/percent2f.mjs"
		...
		// Should work for explicitly-mapped specifiers that happen to have a
		// slash.
		assert_resolve(
		...
		// Should fail for attempting to get a submodule of something not
		// declared with a trailing slash.
		assert!(import_map.resolve("not-a-package/foo", referrer_url).is_err());
	}
    ...
    "imports": {
      ...
      "../test": "/lib/test2.mjs"
		...
		// Should remap URLs that are prefix-matched by keys with trailing
		// slashes.
		assert_resolve(
		...
		// Should use the last entry's address when URL-like specifiers parse to
		// the same absolute URL.
		//
		// NOTE: this works properly because of "preserve_order" feature flag to
		// "serde_json" crate
		assert_resolve(
