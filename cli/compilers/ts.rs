...
use crate::{
	...
	file_fetcher::{SourceFile, SourceFileFetcher},
...
impl CompilerConfig {
	...
	pub fn load(config_path:Option<String>) -> Result<Self, ErrBox> {
		...
		// If `checkJs` is set to true in `compilerOptions` then we're gonna be
...
fn req(
	...
	let j = match (compiler_config.path, compiler_config.content) {
		(Some(config_path), Some(config_data)) => {
			json!({
	...
	pub fn compile_async(
		...
		if self.use_disk_cache {
			...
			if let Some(metadata) = self.get_metadata(&source_file.url) {
				...
				// TODO: it would probably be good idea to make it method
				// implemented on SourceFile
