...
function hasKey(obj: NestedMapping, keys: string[]): boolean {
	let o = obj;
	keys.slice(0, -1).forEach((key: string): void => {
...
export function parse(
	...
	if (options.string !== undefined) {
		...
		stringArgs.filter(Boolean).forEach((key): void => {
	...
	function setKey(obj: NestedMapping, keys: string[], value: unknown): void {
		let o = obj;
		keys.slice(0, -1).forEach((key): void => {
	...
	function setArg(
		...
		(get(aliases, key) || []).forEach((x): void => {
			setKey(argv, x.split("."), value);
		});
	}

	function aliasIsBoolean(key: string): boolean {
		return get(aliases, key)!.some((x): boolean => get(flags.bools, x)!);
	}

	Object.keys(flags.bools).forEach((key): void => {
